...
Optional arguments:
    ...
    -w, --workspace CONSUMER_WORKSPACE
...
go_to_workspace() {
    if [ "$(kubectl api-resources | grep -c ".kcp.dev/")" = "0" ]; then
        printf "'%s' current context is not connected to a kcp instance\n" "$KUBECONFIG" >&2
        exit 1
    fi
    if [ "$(kubectl ws current | cut -d\" -f2)" = "$CONSUMER_WORKSPACE" ]; then
        # Already in the expected workspace
        return
    fi
    if kubectl ws use "$CONSUMER_WORKSPACE" 2>/dev/null; then
        # Workspace exists and user has access
        return
    fi
    # Try to create and access the workspace
    PARENT_WORKSPACE="$(echo "$CONSUMER_WORKSPACE" | sed 's/:[^:]*$//')"
    kubectl ws use "$PARENT_WORKSPACE"
    kubectl ws use "${CONSUMER_WORKSPACE/*:/}" 2>/dev/null || kubectl ws create --enter "${CONSUMER_WORKSPACE/*:/}"
}
