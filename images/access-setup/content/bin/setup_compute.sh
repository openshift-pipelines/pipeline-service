...
parse_args() {
  KUSTOMIZATION=${KUSTOMIZATION:-github.com/avinal/pipeline-service/gitops/compute/pac-manager?ref=avinal/fix-tektonconfig}
  while [[ $# -gt 0 ]]; do
    case "$1" in
    -k | --kubeconfig)
      shift
...
generate_compute_credentials() {
  ...
  echo "resources:
  - github.com/avinal/pipeline-service/gitops/argocd?ref=avinal/fix-tektonconfig
