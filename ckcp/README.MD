...
You can run the deploy.sh script with or without parameters as specified below:
...
$ ./deploy.sh -a pipelines
    #installs openshift-gitops + ckcp + tekton-pipeline + pipelines controller

$ ./deploy.sh -a triggers
    #installs openshift-gitops + ckcp + triggers crds, interceptors and controller 
```
test.sh script runs certain examples from tektoncd repo for pipelines and triggers. You can run the below script only after deploy.sh has run and the required resources are up and running.
